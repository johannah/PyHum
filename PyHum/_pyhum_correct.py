...
def correct(humfile, sonpath, maxW=1000, doplot=1, dofilt=0, correct_withwater=0, ph=7, temp=10, salinity=0, dconcfile=None):
    ...
    meta['maxW'] = maxW
    savemat(os.path.normpath(os.path.join(
        sonpath, base + 'meta.mat')), meta, oned_as='row')
    ...
    if shape_port != '':
        ...
        if os.path.isfile(os.path.normpath(os.path.join(sonpath, base + '_data_port2.dat'))):
            port_fp = pyhumio.get_mmap_data(
                sonpath, base, '_data_port2.dat', 'int16', tuple(shape_port))

        else:
            port_fp = pyhumio.get_mmap_data(
                sonpath, base, '_data_port.dat', 'int16', tuple(shape_port))
    ...
    if shape_star != '':
        if os.path.isfile(os.path.normpath(os.path.join(sonpath, base + '_data_star2.dat'))):
            star_fp = pyhumio.get_mmap_data(
                sonpath, base, '_data_star2.dat', 'int16', tuple(shape_star))

        else:
            star_fp = pyhumio.get_mmap_data(
                sonpath, base, '_data_star.dat', 'int16', tuple(shape_star))
    ...
    shape_star = pyhumio.set_mmap_data(
        sonpath, base, '_data_star_l.dat', 'float32', Zt)
    del Zt

    A = np.squeeze(A)
    # create memory mapped file for A
    shape_A = pyhumio.set_mmap_data(
        sonpath, base, '_data_incidentangle.dat', 'float32', A)
    del A
    ...
    shape_R = pyhumio.set_mmap_data(
        sonpath, base, '_data_range.dat', 'float32', R)
    del R

    TL[np.isnan(TL)] = 0
    TL[TL < 0] = 0
    shape_TL = pyhumio.set_mmap_data(
        sonpath, base, '_data_TL.dat', 'float32', TL)
    del TL

    A_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_incidentangle.dat', 'float32', shape_star)
    TL_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_TL.dat', 'float32', shape_star)

    R_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_range.dat', 'float32', shape_star)
    ...
    if correct_withwater == 1:
        ...
        # create memory mapped file for Z)
        shape_star = pyhumio.set_mmap_data(
            sonpath, base, '_data_star_lw.dat', 'float32', Zt)

    # we are only going to access the portion of memory required
    star_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_star_l.dat', 'float32', shape_star)
    ...
    Zt = correct_scans_lambertian(
        star_fp, A_fp, TL_fp, R_fp, meta['c'], meta['f'], theta, alpha)
    ...
    # create memory mapped file for Z
    shape_star = pyhumio.set_mmap_data(
        sonpath, base, '_data_star_la.dat', 'float32', Zt2)
    del Zt2

    # we are only going to access the portion of memory required
    star_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_star_la.dat', 'float32', shape_star)
    ...
    if correct_withwater == 1:
        ...
        # create memory mapped file for Z)
        shape_port = pyhumio.set_mmap_data(
            sonpath, base, '_data_port_lw.dat', 'float32', Zt)
    ...
    # create memory mapped file for Z
    shape_port = pyhumio.set_mmap_data(
        sonpath, base, '_data_port_l.dat', 'float32', Zt)

    # we are only going to access the portion of memory required
    port_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_port_l.dat', 'float32', shape_port)

    ##Zt = correct_scans(port_fp, A_fp, TL_fp, dofilt)

    # lambertian correction
    Zt = correct_scans_lambertian(
        port_fp, A_fp, TL_fp, R_fp, meta['c'], meta['f'], theta, alpha)
    ...
    # create memory mapped file for Z
    shape_port = pyhumio.set_mmap_data(
        sonpath, base, '_data_port_la.dat', 'float32', Zt2)
    del Zt2

    # we are only going to access the portion of memory required
    port_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_port_la.dat', 'float32', shape_port)

    # do plots of merged scans
    if doplot == 1:
        if correct_withwater == 1:

            port_fpw = pyhumio.get_mmap_data(
                sonpath, base, '_data_port_lw.dat', 'float32', shape_port)

            star_fpw = pyhumio.get_mmap_data(
                sonpath, base, '_data_star_lw.dat', 'float32', shape_star)

            if len(np.shape(star_fpw)) > 2:
                for p in range(len(star_fpw)):
                    plot_merged_scans(
                        port_fpw[p], star_fpw[p], dist_m, shape_port, ft, sonpath, p)
            else:
                plot_merged_scans(port_fpw, star_fpw, dist_m,
                                  shape_port, ft, sonpath, 0)
            ...
            if len(np.shape(star_fp)) > 2:
                for p in range(len(star_fp)):
                    plot_merged_scans(
                        port_fp[p], star_fp[p], dist_m, shape_port, ft, sonpath, p)
            else:
                plot_merged_scans(port_fp, star_fp, dist_m,
                                  shape_port, ft, sonpath, 0)
    ...
    if shape_low != '':
        if os.path.isfile(os.path.normpath(os.path.join(sonpath, base + '_data_dwnlow2.dat'))):
            try:
                low_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnlow2.dat', 'int16', tuple(shape_low))

            except:
                low_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnlow.dat', 'int16', tuple(shape_low))

            finally:
                low_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnlow.dat', 'int16', tuple(shape_hi))

                # if 'shape_hi' in locals():
                #   low_fp = pyhumio.get_mmap_data(sonpath, base, '_data_dwnlow2.dat', 'int16', tuple(shape_hi))
    ...
    if shape_hi != '':
        if os.path.isfile(os.path.normpath(os.path.join(sonpath, base + '_data_dwnhi2.dat'))):
            try:
                hi_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnhi2.dat', 'int16', tuple(shape_hi))

            except:
                hi_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnhi.dat', 'int16', tuple(shape_hi))

            finally:
                hi_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnhi.dat', 'int16', tuple(shape_low))

                # if 'shape_low' in locals():
                #   hi_fp = pyhumio.get_mmap_data(sonpath, base, '_data_dwnhi2.dat', 'int16', tuple(shape_low))
    ...
    if 'low_fp' in locals():
        ...
        # create memory mapped file for Z
        shape_low = pyhumio.set_mmap_data(
            sonpath, base, '_data_dwnlow_l.dat', 'float32', Zt)
        del Zt

        # we are only going to access the portion of memory required
        low_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_dwnlow_l.dat', 'float32', shape_low)
        Zt = correct_scans2(low_fp, TL_fp)

        # create memory mapped file for Z
        shape_low = pyhumio.set_mmap_data(
            sonpath, base, '_data_dwnlow_la.dat', 'float32', Zt)
        del Zt

        # we are only going to access the lowion of memory required
        low_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_dwnlow_la.dat', 'float32', shape_low)

        if doplot == 1:
            if len(np.shape(low_fp)) > 2:
                for p in range(len(low_fp)):
                    plot_dwnlow_scans(
                        low_fp[p], dist_m, shape_low, ft, sonpath, p)
            else:
    ...
    if 'hi_fp' in locals():
        ...
        # create memory mapped file for Z
        shape_hi = pyhumio.set_mmap_data(
            sonpath, base, '_data_dwnhi_l.dat', 'float32', Zt)
        del Zt

        # we are only going to access the portion of memory required
        hi_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_dwnhi_l.dat', 'float32', shape_hi)

        Zt = correct_scans2(hi_fp, TL_fp)

        # create memory mapped file for Z
        shape_hi = pyhumio.set_mmap_data(
            sonpath, base, '_data_dwnhi_la.dat', 'float32', Zt)
        del Zt

        # we are only going to access the hiion of memory required
        hi_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_dwnhi_la.dat', 'float32', shape_hi)

        if doplot == 1:
            if len(np.shape(hi_fp)) > 2:
                for p in range(len(hi_fp)):
                    plot_dwnhi_scans(hi_fp[p], dist_m,
                                     shape_hi, ft, sonpath, p)
            else:
...
def sed_atten(f, conc, dens, d, c):
    ...
    if np.any(conc) > 0:
        ...
        alpha = phi * ((1 / 6) * k**4 * a**3 + k * (sigma - 1) **
                       2 * (s / (s**2 + (sigma + tau)**2))) * 1e4
    ...
    if len(np.shape(fp)) > 2:
        for p in range(len(fp)):
            ...
            for k in range(np.shape(data_dB)[1]):
                try:
                    data_dB[:, k] = np.r_[data_dB[Zbed[k]:, k], np.zeros(
                        (np.shape(data_dB)[0] - np.shape(data_dB[Zbed[k]:, k])[0],))]
                except:
            ...
            if calcR == 1:
                ...
                for k in range(np.shape(r)[1]):
                    try:
                        r[:, k] = np.r_[r[Zbed[k]:, k], np.zeros(
                            (np.shape(r)[0] - np.shape(r[Zbed[k]:, k])[0],))]
                        a[:, k] = np.r_[a[Zbed[k]:, k], np.zeros(
                            (np.shape(a)[0] - np.shape(a[Zbed[k]:, k])[0],))]
                    except:
...
def c_scans(fp, a_fp, TL, dofilt):
    ...
    mg = 10**np.log10(np.asarray(fp * 1 - np.cos(a_fp)
                                 ** 2, 'float32') + 0.001 + TL)
    mg[fp == 0] = np.nan
    mg[mg < 0] = np.nan
    mg[nodata] = np.nan
...
def c_scans_lambertian(fp, a_fp, TL, R, c, f, theta, alpha):
    ...
    phi = ((M * (f * 1000) * a**4) / Rtmp**2) * (2 * jv(1, (2 * np.pi / lam) * a *
                                                        np.sin(np.deg2rad(theta))) / (2 * np.pi / lam) * a * np.sin(np.deg2rad(theta)))**2
...
def plot_merged_scans(dat_port, dat_star, dist_m, shape_port, ft, sonpath, p):
    ...
    # ~os.path.isfile(os.path.normpath(os.path.join(sonpath,'merge_corrected_scan'+str(p)))):
    if 2 > 1:
        if len(shape_port) > 2:
        ...
        fig = plt.figure()
        plt.imshow(np.vstack((np.flipud(np.uint8(dat_port)), np.uint8(dat_star))), cmap='gray', extent=[
                   min(Zdist), max(Zdist), -extent * (1 / ft), extent * (1 / ft)])
...
def plot_dwnlow_scans(dat_dwnlow, dist_m, shape_low, ft, sonpath, p):
    ...
    # ~os.path.isfile(os.path.normpath(os.path.join(sonpath,'dwnlow_corrected_scan'+str(p)))):
    print("plotting dwnlow_scans")
    if len(shape_low) > 2:
        print("plotting shape_low greater than 2")
    ...
    dat_dwnlow8 = dat_dwnlow.astype(np.uint8)
    fig = plt.figure()
    print((min(Zdist), max(Zdist), extent, ft))
    try:
        plt.imshow(dat_dwnlow8, cmap='gray', extent=[
                   min(Zdist), max(Zdist), extent * (1 / ft), 0])
        plt.axis('normal')
        plt.axis('tight')
    except Exception as e:
        print(('Exception in dwnlow', e))
        plt.imshow(dat_dwnlow8)
...
def plot_dwnhi_scans(dat_dwnhi, dist_m, shape_hi, ft, sonpath, p):
    # ~os.path.isfile(os.path.normpath(os.path.join(sonpath,'dwnhi_corrected_scan'+str(p)))):
    if len(shape_hi) > 2:
    ...
    dat_dwnhi8 = dat_dwnhi.astype(np.uint8)
    fig = plt.figure()
    print((min(Zdist), max(Zdist), extent, ft))
    try:
        plt.imshow(dat_dwnhi8, cmap='gray', extent=[
                   min(Zdist), max(Zdist), extent * (1 / ft), 0])
        plt.axis('normal')
        plt.axis('tight')
    except Exception as e:
        print(('exception in dwnhi', e))
        plt.imshow(dat_dwnhi8)
