...
def auto_bedpick(ft, dep_m, chunkmode, port_fp, c):
    ...
    if chunkmode != 4:
        ...
        # JRH - got slice error  so trying to fix
        for k in range(len(port_fp)):
            minval = np.max([0, int(np.min(bed) - buff)])
            maxval = int(np.max(bed) + buff)
            print(('reading port index: %s from %s to %s' % (k, minval, maxval)))
            imu.append(port_fp[k][minval:maxval, :])

        imu = np.hstack(imu)
    else:
        minval = np.max([0, int(np.min(bed) - buff)])
        maxval = int(np.max(bed) + buff)
        imu.append(port_fp[minval:maxval, :])
    ...
    try:
        imu = median_filter(
            imu, (np.shape(imu)[0] / 100, np.shape(imu)[1] / 100))
    except:
        imu = median_filter(
            imu, (np.shape(imu)[0] / 2, np.shape(imu)[1] / 100))
    ...
    # if standard deviation of auto bed pick is too small, then use acoustic
    # bed pick
    if np.std(x) < 5:
...
def get_bearing(calc_bearing, filt_bearing, lat, lon, heading):  # cog
    ...
    if calc_bearing == 1:
        ...
        for k in range(len(lat) - 1):
            bearing[k] = bearingBetweenPoints(
                lat[k], lat[k + 1], lon[k], lon[k + 1])
        #del lat, lon
    ...
    # if stdev in heading is large, there's probably noise that needs to be
    # filtered out
...
def bearingBetweenPoints(pos1_lat, pos2_lat, pos1_lon, pos2_lon):
    ...
    # orig #bearing = np.arctan2(np.cos(lat1) * np.sin(lat2) - (np.sin(lat1) *
    # np.cos(lat2) * np.cos(diffLong)), np.sin(diffLong) * np.cos(lat2))

    bearing = np.arctan2(np.sin(diffLong) * np.cos(lat2), np.cos(lat1)
                         * np.sin(lat2) - np.sin(lat1) * np.cos(lat2) * np.cos(diffLong))
