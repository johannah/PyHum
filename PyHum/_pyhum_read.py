...
#          print "Heading based on course-over-ground"
    ...
    try:
        o = Parallel(n_jobs=np.min([len(sonfiles), cpu_count()]), verbose=0)(delayed(
            getscans)(sonfiles[k], humfile, c, model, cs2cs_args) for k in range(len(sonfiles)))
        X, Y, A, B = list(zip(*o))
    ...
    try:
        es = humutils.runningMeanFast(
            metadat['e'][:nrec], len(metadat['e'][:nrec]) / 100)
        ns = humutils.runningMeanFast(
            metadat['n'][:nrec], len(metadat['n'][:nrec]) / 100)
    except:
        es = metadat['e'][:nrec]
        ns = metadat['n'][:nrec]
    ...
    metadat['heading'] = humutils.get_bearing(
        calc_bearing, filt_bearing, metadat['lat'], metadat['lon'], metadat['instr_heading'])  # cog
    ...
    if data_port != '':
        ...
        Zt, ind_port = makechunks_scan(
            chunkmode, chunkval, metadat, data_port, 0)
        del data_port

        # create memory mapped file for Z
        shape_port = pyhumio.set_mmap_data(
            sonpath, base, '_data_port.dat', 'int16', Zt)

        # we are only going to access the portion of memory required
        port_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_port.dat', 'int16', shape_port)
    ...
    if data_star != '':
        ...
        Zt, ind_star = makechunks_scan(
            chunkmode, chunkval, metadat, data_star, 1)
        del data_star

        # create memory mapped file for Z
        shape_star = pyhumio.set_mmap_data(
            sonpath, base, '_data_star.dat', 'int16', Zt)

        star_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_star.dat', 'int16', shape_star)
    ...
    if 'star_fp' in locals() and 'port_fp' in locals():
        ...
        if np.shape(star_fp) != np.shape(port_fp):
            ...
            if np.shape(port_fp[0])[1] > np.shape(star_fp[0])[1]:
                ...
                # create memory mapped file for Z
                shape_port = pyhumio.set_mmap_data(
                    sonpath, base, '_data_port2.dat', 'int16', tmp2)
                #shape_star = shape_port.copy()
                shape_star = tuple(np.asarray(shape_port).copy())

                # we are only going to access the portion of memory required
                port_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_port2.dat', 'int16', shape_port)
                ...
                # create memory mapped file for Z
                shape_port = pyhumio.set_mmap_data(
                    sonpath, base, '_data_star2.dat', 'int16', tmp2)
                #shape_star = shape_port.copy()
                shape_star = tuple(np.asarray(shape_port).copy())

                # we are only going to access the portion of memory required
                star_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_star2.dat', 'int16', shape_star)
    ...
    if data_dwnlow != '':
        ...
        Zt, ind_low = makechunks_scan(
            chunkmode, chunkval, metadat, data_dwnlow, 2)
        del data_dwnlow

        # create memory mapped file for Z
        shape_low = pyhumio.set_mmap_data(
            sonpath, base, '_data_dwnlow.dat', 'int16', Zt)

        # we are only going to access the portion of memory required
        dwnlow_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_dwnlow.dat', 'int16', shape_low)
    ...
    if data_dwnhi != '':
        ...
        Zt, ind_hi = makechunks_scan(
            chunkmode, chunkval, metadat, data_dwnhi, 3)
        del data_dwnhi

        # create memory mapped file for Z
        shape_hi = pyhumio.set_mmap_data(
            sonpath, base, '_data_dwnhi.dat', 'int16', Zt)

        dwnhi_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_dwnhi.dat', 'int16', shape_hi)
    ...
    if 'dwnhi_fp' in locals() and 'dwnlow_fp' in locals():
        ...
        if (np.shape(dwnhi_fp) != np.shape(dwnlow_fp)) and (chunkmode != 4):
            ...
            if np.shape(dwnhi_fp[0])[1] > np.shape(dwnlow_fp[0])[1]:
                ...
                # create memory mapped file for Z
                shape_low = pyhumio.set_mmap_data(
                    sonpath, base, '_data_dwnhi2.dat', 'int16', tmp2)
                #shape_hi = shape_low.copy()
                shape_hi = tuple(np.asarray(shape_low).copy())

                # we are only going to access the portion of memory required
                dwnhi_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnhi2.dat', 'int16', shape_hi)
                ...
                # create memory mapped file for Z
                shape_low = pyhumio.set_mmap_data(
                    sonpath, base, '_data_dwnlow2.dat', 'int16', tmp2)
                #shape_hi = shape_low.copy()
                shape_hi = tuple(np.asarray(shape_low).copy())

                # we are only going to access the portion of memory required
                dwnlow_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_dwnlow2.dat', 'int16', shape_low)
    ...
    if 'port_fp' in locals() and 'star_fp' in locals():
        ...
        # if not
        # os.path.isfile(os.path.normpath(os.path.join(sonpath,base+'meta.mat'))):
        if 2 > 1:
            if bedpick == 1:  # auto

                x, bed = humutils.auto_bedpick(
                    ft, dep_m, chunkmode, port_fp, c)
                ...
                if doplot == 1:
                    if chunkmode != 4:
                        for k in range(len(star_fp)):
                            plot_2bedpicks(port_fp[k], star_fp[k], bed[ind_port[-1] * k:ind_port[-1] * (k + 1)], dist_m[ind_port[-1] * k:ind_port[-1] * (
                                k + 1)], x[ind_port[-1] * k:ind_port[-1] * (k + 1)], ft, shape_port, sonpath, k, chunkmode)
                    else:
                        plot_2bedpicks(port_fp, star_fp, bed, dist_m,
                                       x, ft, shape_port, sonpath, 0, chunkmode)

                # 'real' bed is estimated to be the minimum of the two
                bed = np.min(
                    np.vstack((bed[:nrec], np.squeeze(x[:nrec]))), axis=0)
                ...
                pts1 = plt.ginput(n=300, timeout=30)
                # map applies the function passed as
                x1 = [x[0] for x in pts1]
                # first parameter to each element of pts
                y1 = [x[1] for x in pts1]
                plt.close()
                del fig

                if x1 != []:  # if x1 is not empty
                    tree = KDTree(list(zip(np.arange(1, len(bed)), bed)))
                    try:
                        dist, inds = tree.query(
                            list(zip(x1, y1)), k=100, eps=5, n_jobs=-1)
                    except:
                        dist, inds = tree.query(list(zip(x1, y1)), k=100, eps=5)
                ...
                if chunkmode != 4:
                    for k in range(len(port_fp)):
                        input("Bed picking " + str(k + 1) + " of " + str(len(port_fp)) +
            ...
            if doplot == 1:
                if chunkmode != 4:
                    for k in range(len(star_fp)):
                        plot_bedpick(port_fp[k], star_fp[k], (1 / ft) * bed[ind_port[-1] * k:ind_port[-1] * (
                            k + 1)], dist_m[ind_port[-1] * k:ind_port[-1] * (k + 1)], ft, shape_port, sonpath, k, chunkmode)
                else:
                    plot_bedpick(port_fp, star_fp, (1 / ft) * bed,
                                 dist_m, ft, shape_port, sonpath, 0, chunkmode)
    ...
    writer = csv.writer(f)
    writer.writerow(('longitude', 'latitude', 'easting', 'northing',
                     'depth (m)', 'distance (m)', 'instr. heading (deg)', 'heading (deg.)'))
    for i in range(0, nrec):
        writer.writerow((float(lon[i]), float(lat[i]), float(es[i]), float(ns[i]), float(
            dep_m[i]), float(dist_m[i]), float(metadat['instr_heading'][i]), float(metadat['heading'][i])))
    f.close()
...
def plot_2bedpicks(dat_port, dat_star, Zbed, Zdist, Zx, ft, shape_port, sonpath, k, chunkmode):
    ...
    plt.subplot(2, 2, 1)
    ax = plt.gca()
    ax.imshow(np.flipud(dat_port), cmap='gray', extent=[
              min(Zdist), max(Zdist), 0, extent * (1 / ft)], origin='upper')  # im =
    ...
    plt.subplot(2, 2, 3)
    ax = plt.gca()
    ax.imshow(dat_star, cmap='gray', extent=[min(Zdist), max(
        Zdist), extent * (1 / ft), 0], origin='upper')  # im =
    plt.ylabel('Range (m)')
    ...
    axR.imshow(dat_star, cmap='gray', extent=[min(Zdist), max(
        Zdist), extent * (1 / ft), 0], origin='upper')
...
def plot_bedpick(dat_port, dat_star, Zbed, Zdist, ft, shape_port, sonpath, k, chunkmode):
    ...
    fig = plt.figure()
    plt.subplot(2, 2, 1)
    plt.imshow(np.flipud(dat_star), cmap='gray', extent=[
               min(Zdist), max(Zdist), 0, extent * (1 / ft)], origin='upper')
    ...
    plt.subplot(2, 2, 3)
    plt.imshow(dat_port, cmap='gray', extent=[min(Zdist), max(
        Zdist), extent * (1 / ft), 0], origin='upper')
    plt.plot(np.linspace(min(Zdist), max(Zdist), len(Zbed)), Zbed, 'r')
    plt.axis('normal')
    plt.axis('tight')
...
def sliding_window(a, ws, ss=None, flatten=True):
    ...
    shape_tmp = pyhumio.set_mmap_data('', '', 'tmp.dat', 'float32', a)
    del a
    a = pyhumio.get_mmap_data('', '', 'tmp.dat', 'float32', shape_tmp)
...
if __name__ == '__main__':
    ...
    read(humfile, sonpath, cs2cs_args, c, draft, doplot, t, f, bedpick,
