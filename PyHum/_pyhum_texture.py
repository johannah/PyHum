...
def texture(humfile, sonpath, win=100, shift=10, doplot=1, density=50, numclasses=4, maxscale=20, notes=4):
    ...
    if shape_port != '':
        ...
        port_fp2 = pyhumio.get_mmap_data(
    ...
    if shape_star != '':
        ...
        star_fp2 = pyhumio.get_mmap_data(
    ...
    if len(shape_star) > 2:
        ...
        for p in range(len(port_fp)):
            ...
            Z, ind = humutils.sliding_window_sliced(np.vstack(
                (np.flipud(port_fp[p]), star_fp[p])), density, (win, win), (shift, shift))

            Snn = get_srt(Z, ind, maxscale, notes, win)  # , density)

            # replace nans using infilling algorithm
            rn = replace_nans.RN(Snn.astype('float64'),
                                 1000, 0.01, 2, 'localmean')
            Snn = rn.getdata()
            del rn
            ...
            rn = replace_nans.RN(R.astype('float64'),
                                 1000, 0.01, 2, 'localmean')
            R = rn.getdata()
            del rn
        ...
        Z, ind = humutils.sliding_window_sliced(
            np.vstack((np.flipud(port_fp), star_fp)), density, (win, win), (shift, shift))
    ...
    if doplot == 1:
        ...
        if len(shape_star) > 2:
            for p in range(len(star_fp)):
                plot_class(
                    dist_m, shape_port, port_fp[p], star_fp[p], class_fp[p], ft, humfile, sonpath, base, p)
        else:
            plot_class(dist_m, shape_port, port_fp, star_fp,
                       class_fp, ft, humfile, sonpath, base, 0)

        if len(shape_star) > 2:
            for p in range(len(star_fp)):
                plot_contours(
                    dist_m, shape_port, class_fp[p], ft, humfile, sonpath, base, numclasses, p)
        else:
            plot_contours(dist_m, shape_port, class_fp, ft,
    ...
    if len(shape_star) > 2:
        ...
        kclass_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_kclass.dat', 'float32', tuple(shape))
    ...
    if doplot == 1:
        ...
        if len(shape_star) > 2:
            for p in range(len(star_fp)):
                plot_kmeans(
                    dist_m, shape_port, port_fp[p], star_fp[p], kclass_fp[p], ft, humfile, sonpath, base, p)
        else:
            plot_kmeans(dist_m, shape_port, port_fp, star_fp,
                        kclass_fp, ft, humfile, sonpath, base, 0)
...
def get_srt(Z, ind, maxscale, notes, win):  # , density):
    try:
        # print "%s windows to process with a density of %s" % (str(len(Z)),
        # str(density)) #% (str(len(Z)), str(density))
        print(("%s windows to process" % (str(len(Z)))))
        # do the wavelet clacs and get the stats
        d = Parallel(n_jobs=-1, verbose=0)(delayed(parallel_me)
                                           (Z[k], maxscale, notes, win) for k in range(len(Z)))  # density
    except:
...
def plot_class(dist_m, shape_port, dat_port, dat_star, dat_class, ft, humfile, sonpath, base, p):
    ...
    plt.subplot(2, 1, 1)
    ax = plt.gca()
    im = ax.imshow(np.vstack((np.flipud(dat_port), dat_star)), cmap='gray', extent=[
                   min(Zdist), max(Zdist), -extent * (1 / ft), extent * (1 / ft)], origin='upper')
    ...
    plt.subplot(2, 1, 2)
    ax = plt.gca()
    plt.imshow(np.vstack((np.flipud(dat_port), dat_star)), cmap='gray', extent=[
               min(Zdist), max(Zdist), -extent * (1 / ft), extent * (1 / ft)], origin='upper')
    im = ax.imshow(dat_class, alpha=0.5, extent=[min(Zdist), max(Zdist), -extent * (
        1 / ft), extent * (1 / ft)], origin='upper', cmap='YlOrRd')  # , vmin=0.5, vmax=3)
    plt.ylabel('Horizontal distance (m)')
...
def plot_contours(dist_m, shape_port, dat_class, ft, humfile, sonpath, base, numclasses, p):
    ...
    levels = np.linspace(np.nanmin(dat_class),
                         np.nanmax(dat_class), numclasses + 1)

    fig = plt.figure()
    plt.subplot(2, 1, 1)
    ax = plt.gca()
    CS = plt.contourf(dat_class.astype('float64'), levels, extent=[min(Zdist), max(
        Zdist), -extent * (1 / ft), extent * (1 / ft)], cmap='YlOrRd', origin='upper')  # vmin=0.5, vmax=3)
    plt.ylabel('Horizontal distance (m)')
    plt.xlabel('Distance along track (m)')
    plt.axis('tight')
...
def plot_kmeans(dist_m, shape_port, dat_port, dat_star, dat_kclass, ft, humfile, sonpath, base, p):
    ...
    CS = plt.contourf(np.flipud(dat_kclass), alpha=0.4, extent=[min(Zdist), max(
        Zdist), -extent * (1 / ft), extent * (1 / ft)], origin='upper', cmap='YlOrRd')  # , levels, vmin=0.5, vmax=3)
    plt.ylabel('Horizontal distance (m)')
    plt.xlabel('Distance along track (m)')
    plt.axis('tight')
...
if __name__ == '__main__':
    ...
    texture(humfile, sonpath, win, shift, doplot,
            density, numclasses, maxscale, notes)
    ...
    #tmpfp = pyhumio.get_mmap_data(sonpath, base, '_tmp.dat', 'float32', tmpshape)
...
#            del SRT
    ...
    # except:
    #   print "memory error: trying serial"
    # d = Parallel(n_jobs = 1, verbose=0)(delayed(parallel_me)(Z[k].T,
    # maxscale, notes, win) for k in xrange(len(Z))) #density
