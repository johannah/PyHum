...
def mosaic(humfile, sonpath, cs2cs_args="epsg:26949", res=99, nn=5, noisefloor=10, weight=1):
    ...
    if not os.path.isfile(os.path.normpath(os.path.join(sonpath, base + "S.p"))):
        ...
        if len(shape_star) > 2:
            for p in range(len(star_fp)):
                ...
                write_points(e, n, t, d, dat_port, dat_star, data_R,
                             pix_m, res, cs2cs_args, sonpath, p, c, dx)
                inputfiles.append(os.path.normpath(os.path.join(
                    sonpath, 'x_y_ss_raw' + str(p) + '.asc')))
        else:
            p = 0
            print("writing chunk %s " % (str(p)))
            write_points(esi, nsi, theta, dist_tvg, port_fp, star_fp,
                         R_fp, meta['pix_m'], res, cs2cs_args, sonpath, 0, c, dx)
            inputfiles.append(os.path.normpath(os.path.join(
                sonpath, 'x_y_ss_raw' + str(p) + '.asc')))
        ...
        pickle.dump(S, open(os.path.normpath(
            os.path.join(sonpath, base + "S.p")), "wb"))
        del S
        pickle.dump(D, open(os.path.normpath(
            os.path.join(sonpath, base + "D.p")), "wb"))
        del D
        pickle.dump(t, open(os.path.normpath(
            os.path.join(sonpath, base + "t.p")), "wb"))
        del t
        pickle.dump(i, open(os.path.normpath(
            os.path.join(sonpath, base + "i.p")), "wb"))
        del i

        pickle.dump(X, open(os.path.normpath(
            os.path.join(sonpath, base + "X.p")), "wb"))
        del X
        pickle.dump(Y, open(os.path.normpath(
            os.path.join(sonpath, base + "Y.p")), "wb"))
        del Y
        pickle.dump(R, open(os.path.normpath(
            os.path.join(sonpath, base + "R.p")), "wb"))
        pickle.dump(h, open(os.path.normpath(
            os.path.join(sonpath, base + "h.p")), "wb"))

        # grazing angle
        g = np.arctan(R.flatten(), h.flatten())
        pickle.dump(g, open(os.path.normpath(
            os.path.join(sonpath, base + "g.p")), "wb"))
        del g, R, h
    ...
    if res == 99:
        ...
        # prepare grids
        R = pickle.load(open(os.path.normpath(
            os.path.join(sonpath, base + "R.p")), "rb"))
    ...
    X = pickle.load(open(os.path.normpath(
        os.path.join(sonpath, base + "X.p")), "rb"))
    Y = pickle.load(open(os.path.normpath(
        os.path.join(sonpath, base + "Y.p")), "rb"))

    humlon, humlat = trans(X, Y, inverse=True)

    grid_x, grid_y = np.meshgrid(np.arange(np.min(X), np.max(
        X), resg), np.arange(np.min(Y), np.max(Y), resg))
    ...
    try:
        dist, inds = tree.query(
            list(zip(grid_x.flatten(), grid_y.flatten())), k=nn, n_jobs=-1)
    except:
        # print ".... update your scipy installation to use faster kd-tree"
        dist, inds = tree.query(list(zip(grid_x.flatten(), grid_y.flatten())), k=nn)

    #del grid_x, grid_y

    if weight == 1:
        g = pickle.load(open(os.path.normpath(
            os.path.join(sonpath, base + "g.p")), "rb"))
        w = g[inds] + 1.0 / dist**2
        del g
    elif weight == 2:
        g = pickle.load(open(os.path.normpath(
    ...
    S = pickle.load(open(os.path.normpath(
        os.path.join(sonpath, base + "S.p")), "rb"))
    ...
    if nn == 1:
        ...
        if weight < 4:
            Sdat_g = (np.nansum(w * S.flatten()
                                [inds], axis=1) / np.nansum(w, axis=1)).reshape(shape)
        else:
    ...
    dat = Sdat_g.copy()
    dat[dist > 1] = 0
    dat2 = replace_nans.RN(dat.astype('float64'), 1000,
                           0.01, 2, 'localmean').getdata()
    dat2[dat == 0] = np.nan
    del dat
    ...
    if Sdat_gm.size > 25000000:
        ...
        map.pcolormesh(gx[::5, ::5], gy[::5, ::5], Sdat_gm[::5, ::5],
                       cmap='gray', vmin=np.nanmin(Sdat_gm), vmax=np.nanmax(Sdat_gm))
    else:
        map.pcolormesh(gx, gy, Sdat_gm, cmap='gray',
                       vmin=np.nanmin(Sdat_gm), vmax=np.nanmax(Sdat_gm))
...
def getxys(inputfiles):
    o = Parallel(n_jobs=-1, verbose=0)(delayed(getdat)
                                       (inputfiles[k], k) for k in range(len(inputfiles)))
    # return zip(*o)
    tmp = np.vstack(o)
