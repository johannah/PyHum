...
def rmshadows(humfile, sonpath, win=31, shadowmask=0, doplot=1):
    ...
    if shape_port != '':
        ...
        with open(os.path.normpath(os.path.join(sonpath, base + '_data_port_la.dat')), 'r') as ff:
            port_fp = np.memmap(ff, dtype='float32',
                                mode='r', shape=tuple(shape_port))
    ...
    if shape_star != '':
        ...
        with open(os.path.normpath(os.path.join(sonpath, base + '_data_star_la.dat')), 'r') as ff:
            star_fp = np.memmap(ff, dtype='float32',
                                mode='r', shape=tuple(shape_star))
    ...
    if shadowmask == 1:  # manual
        ...
        if len(np.shape(star_fp)) > 2:
            for p in range(len(star_fp)):
                ...
                plt.axis('normal')
                plt.axis('tight')
                # it will wait for 200 clicks or 30 seconds
                ...
                shoreline_star = np.interp(
                    np.r_[:np.shape(star_fp[p])[1]], x1, y1)
                plt.close()
                del fig
        ...
        if len(np.shape(star_fp)) > 2:
            for p in range(len(port_fp)):
                ...
                # first parameter to each element of pts
                y1 = [x[1] for x in pts1]
                shoreline_port = np.interp(
                    np.r_[:np.shape(port_fp[p])[1]], x1, y1)
                plt.close()
                del fig
        ...
        if len(np.shape(star_fp)) > 2:
            for p in range(len(star_fp)):
                ...
                # only 8bit precision needed
                mask = np.asarray(merge != 0, 'int8')
                ...
                try:  # parallel processing with all available cores
                    w = Parallel(
                        n_jobs=-1, verbose=0)(delayed(parallel_me)(Z[k]) for k in range(len(Z)))
                except:  # fall back to serial
                    w = Parallel(n_jobs=1, verbose=0)(
                        delayed(parallel_me)(Z[k]) for k in range(len(Z)))
                ...
                bw2 = binary_dilation(binary_erosion(
                    bw, structure=np.ones((3, 3))), structure=np.ones((13, 13)))
                ...
                # fill holes
                bw2 = binary_fill_holes(
                    bw2, structure=np.ones((win, win))).astype(int)
                merge2 = grey_erosion(merge, structure=np.ones((win, win)))

                #del bw
                # bw2 = np.asarray(bw2!=0,'int8') # we only need 8 bit
                # precision

                # we only need 8 bit precision
                bw2 = np.asarray(bw != 0, 'int8')
                del bw
                ...
                if doplot == 1:
                    ...
                    fig = plt.figure()
                    plt.imshow(merge, cmap='gray', extent=[min(Zdist), max(
                        Zdist), -extent * (1 / ft), extent * (1 / ft)])
                    ...
                    plt.axis('tight')
                    custom_save(
                        sonpath, 'merge_corrected_rmshadow_scan' + str(p))
                    del fig
