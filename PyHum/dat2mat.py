...
def do_savemat(fp, datfile):
    for k in range(len(fp)):
        savemat(datfile.split('.dat')[0] + '_' + str(k) + '.mat', mdict={
                os.path.basename(datfile).split('.dat')[0]: np.squeeze(fp[k])})


Tk().withdraw()  # we don't want a full GUI, so keep the root window from appearing
humfile = askopenfilename(title="Select the .DAT file?",
                          filetypes=[("DAT file:", "*.DAT")])

datfiles = askopenfilename(title="Which file do you wish to convert?", filetypes=[
                           ("dat file:", "*.dat")], multiple=True)
