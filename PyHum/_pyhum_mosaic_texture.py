...
def mosaic_texture(humfile, sonpath, cs2cs_args="epsg:26949", res=99, nn=5, weight=1):
    ...
    fig.savefig(os.path.normpath(os.path.join(
        sonpath, 'class_overlay1.png')), transparent=True, format='png')
    ...
    fig.savefig(os.path.normpath(os.path.join(
        sonpath, 'class_legend.png')), transparent=False, format='png')
    ...
    humutils.make_kml(llcrnrlon=glon.min(), llcrnrlat=glat.min(),
                      ...
                      figs=[os.path.normpath(os.path.join(
                          sonpath, 'class_overlay1.png'))],
                      colorbar=os.path.normpath(
                          os.path.join(sonpath, 'class_legend.png')),
                      kmzfile=os.path.normpath(os.path.join(
                          sonpath, 'class_GroundOverlay.kmz')),
    ...
    if Sdat_gm.size > 25000000:
        ...
        map.pcolormesh(gx, gy, Sdat_gm, vmin=np.nanmin(
            Sdat_gm), vmax=np.nanmax(Sdat_gm))
...
def write_points(e, n, t, d, dat_port, dat_star, data_R, pix_m, res, cs2cs_args, sonpath, p, c, dx):
    ...
    )), humutils.ascol(D.flatten()), humutils.ascol(R.flatten()), humutils.ascol(h.flatten()), humutils.ascol(t.flatten()))))
