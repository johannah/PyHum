...
def gui():
    ...
    # create notebook
    # create a new frame slaved to master
    demoPanel = tkinter.Frame(master, name='demo')
    ...
    # make panel for logo
    # , cursor = "cross")
    image_panel = tkinter.Canvas(frame, width=250, height=232)
    image_panel.grid(column=0, row=2)
    image_panel.configure(background='black')

    show_image = ImageTk.PhotoImage(Image.open(
        PyHum.__path__[0] + os.sep + "pyhum_logo_white_sm.png"))
    # show the panel
    ...
    def hello1_alt():
        try:
            ...
            T.tag_configure('bold_italics', font=(
    ...
    MSG1_btn = tkinter.Button(
        read_frame, text="Instructions", command=hello1_alt)
    ...
    #=======================
    # menu for model
    self.mb = tkinter.Menubutton(
        read_frame, text="Humminbird model", relief=tkinter.RAISED)
    self.mb.grid(column=0, row=2, pady=(2, 4))
    self.mb.menu = tkinter.Menu(
        self.mb, tearoff=0, background='PaleVioletRed1', fg="black")

    self.mb.menu.add_command(
        label="997", command=lambda v=1: _SetModel(master, v))
    self.mb.menu.add_command(
        label="998", command=lambda v=2: _SetModel(master, v))
    self.mb.menu.add_command(
        label="1198", command=lambda v=3: _SetModel(master, v))
    self.mb.menu.add_command(
        label="1199", command=lambda v=4: _SetModel(master, v))
    self.mb["menu"] = self.mb.menu
    self.mb.configure(background='thistle3', fg="black")

    #=======================
    # menu for bedpick
    self.bb = tkinter.Menubutton(
        read_frame, text="Bed pick type", relief=tkinter.RAISED)
    self.bb.grid(column=1, row=2, pady=(2, 4))
    self.bb.menu = tkinter.Menu(
        self.bb, tearoff=0, background='PaleVioletRed1', fg="black")

    self.bb.menu.add_command(
        label="Auto", command=lambda v=1: _SetBedPick(master, v))
    self.bb.menu.add_command(
        label="Manual", command=lambda v=0: _SetBedPick(master, v))
    self.bb.menu.add_command(
        label="Hybrid", command=lambda v=2: _SetBedPick(master, v))
    ...
    cscale = tkinter.Scale(read_frame, variable=self.cvar, from_=1440,
                           to=1510, resolution=5, tickinterval=10, label='Sound Velocity [m/s]')
    cscale.set(1450)
    ...
    tscale = tkinter.Scale(read_frame, variable=self.tvar, from_=.1, to=.5,
                           resolution=.001, tickinterval=.1, label='Transducer Length [m]')
    tscale.set(0.108)
    ...
    dscale = tkinter.Scale(read_frame, variable=self.dvar, from_=0,
                           to=1, resolution=.01, tickinterval=.2, label='Draft [m]')
    dscale.set(0.3)
    ...
    self.epsg1_entry = tkinter.Entry(
        read_frame, width=30, textvariable=self.epsg1)
    # , columnspan = 2,sticky = 'EW')
    self.epsg1_entry.grid(column=0, row=5, pady=(2, 4))
    self.epsg1_entry.bind(
        "<Return>", lambda epsg=self.epsg1.get(): _OnPressEnter1(self))
    self.epsg1.set("epsg:26949")
    self.epsg1_entry.configure(background='thistle3', fg="black")

    #=======================
    # menu for chunk
    self.cb = tkinter.Menubutton(
        read_frame, text="chunk argument", relief=tkinter.RAISED)
    self.cb.grid(column=1, row=5, pady=(2, 4))
    self.cb.menu = tkinter.Menu(
        self.cb, tearoff=0, background='PaleVioletRed1', fg="black")

    self.cb.menu.add_command(
        label="d50", command=lambda v=1: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="d100", command=lambda v=2: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="d200", command=lambda v=3: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="d400", command=lambda v=4: _SetChunk(master, v))

    self.cb.menu.add_command(
        label="p1000", command=lambda v=5: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="p5000", command=lambda v=6: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="p10000", command=lambda v=7: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="p20000", command=lambda v=8: _SetChunk(master, v))

    self.cb.menu.add_command(
        label="h5", command=lambda v=9: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="h10", command=lambda v=10: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="h20", command=lambda v=11: _SetChunk(master, v))
    self.cb.menu.add_command(
        label="h40", command=lambda v=12: _SetChunk(master, v))
    ...
    flip_entry = tkinter.Checkbutton(
        read_frame, text='Flip Port/Star', variable=self.flipvar)
    ...
    calchead_entry = tkinter.Checkbutton(
        read_frame, text='Calculate Heading', variable=self.calcheadvar)
    ...
    filthead_entry = tkinter.Checkbutton(
        read_frame, text='Filter Heading', variable=self.filtheadvar)
    ...
    def _proc(self):
        ...
        # do stuff here
        PyHum.read(str(self.DATfilename.get()), os.path.dirname(self.SONfiles[0]), str(self.epsg1.get()), self.cvar.get(), self.dvar.get(
        ), self.doplot, self.tvar.get(), self.bedpick, self.flipvar.get(), self.model, self.calcheadvar.get(), self.filtheadvar.get(), self.chunk)

        self.update()
        messagebox.showinfo("Done!", "Read module finished")

    #=======================
    def _get_DAT(master, v):
        self.DATfile = askopenfilename(
            filetypes=[("DAT files", "*.DAT")], multiple=False)
    ...
    def _get_SON(master, v):
        self.SONfiles = askopenfilename(
            filetypes=[("SON files", "*.SON")], multiple=True)
    ...
    def _SetChunk(master, v):
        if v == 1:
            ...
            self.chunk = 'd100'
            print("chunk is 100 m")
        elif v == 3:
            self.chunk = 'd200'
            print("chunk is 200 m")
        elif v == 4:
            self.chunk = 'd400'
            print("chunk is 400 m")
        elif v == 5:
            self.chunk = 'p1000'
            print("chunk is 1000 pings")
        elif v == 6:
            self.chunk = 'p5000'
            print("chunk is 5000 pings")
        elif v == 7:
            self.chunk = 'p10000'
            print("chunk is 10,000 pings")
        elif v == 8:
            self.chunk = 'p20000'
            print("chunk is 20,000 pings")
        elif v == 9:
            self.chunk = 'h5'
            print("chunk is a 5 degree heading change")
        elif v == 10:
            self.chunk = 'h10'
            print("chunk is a 10 degree heading change")
        elif v == 11:
            self.chunk = 'h20'
            print("chunk is a 20 degree heading change")
        elif v == 12:
            self.chunk = 'h40'
            print("chunk is a 40 degree heading change")
    ...
    MSG2_btn = tkinter.Button(
        correct_frame, text="Instructions", command=hello2_alt)
    ...
    Wscale = tkinter.Scale(correct_frame, variable=self.Wvar, from_=500,
                           to=1500, resolution=100, tickinterval=200, label='RMS Output [W]')
    Wscale.set(1000)
    ...
    Pscale = tkinter.Scale(correct_frame, variable=self.Pvar,
                           from_=4, to=8, resolution=.1, tickinterval=1, label='pH')
    Pscale.set(7)
    ...
    # temp
    self.Tvar = tkinter.DoubleVar()
    Tscale = tkinter.Scale(correct_frame, variable=self.Tvar, from_=3,
                           to=25, resolution=1, tickinterval=5, label='Temp [degC]')
    Tscale.set(10)
    ...
    Sscale = tkinter.Scale(correct_frame, variable=self.Svar, from_=0,
                           to=35, resolution=1, tickinterval=5, label='Salinity [ppt]')
    Sscale.set(0)
    ...
    dofilt_entry = tkinter.Checkbutton(
        correct_frame, text='Do Filter', variable=self.dofiltvar)
    ...
    water_entry = tkinter.Checkbutton(
        correct_frame, text='Retain Water Column', variable=self.watervar)
    ...
    def _proc2(self):
        ...
        # do stuff here
        PyHum.correct(str(self.DATfilename.get()), os.path.dirname(self.SONfiles[0]), self.Wvar.get(), self.doplot, self.dofiltvar.get(
        ), self.watervar.get(), self.Pvar.get(), self.Tvar.get(), self.Svar.get(), self.sedfilename.get())

        self.update()
        messagebox.showinfo("Done!", "Correct module finished")

    #=======================
    def _get_sedfile(master, v):
        self.sedfile = askopenfilename(
            filetypes=[("Sediment file", "*.txt")], multiple=False)
    ...
    MSG3_btn = tkinter.Button(
        rmshadows_frame, text="Instructions", command=hello3_alt)
    ...
    # get dat file
    datVar = tkinter.StringVar()
    self.rmshadows_dat_btn = tkinter.Button(
        rmshadows_frame, text='Get DAT file', underline=0, command=lambda v=datVar: _get_DAT(master, v))
    ...
    Winscale = tkinter.Scale(rmshadows_frame, variable=self.Winvar, from_=5,
                             to=500, resolution=1, tickinterval=50, label='Window Size [pixels]')
    Winscale.set(31)
    ...
    manmask_entry = tkinter.Checkbutton(
        rmshadows_frame, text='Manual Mask', variable=self.manmaskvar)
    ...
    def _proc3(self):
        ...
        # do stuff here
        PyHum.rmshadows(str(self.DATfilename.get()), os.path.dirname(
            self.SONfiles[0]), self.Winvar.get(), self.manmaskvar.get(), self.doplot)
        self.update()
        messagebox.showinfo("Done!", "Shadow removal module finished")
    ...
    MSG4_btn = tkinter.Button(
        map_frame, text="Instructions", command=hello4_alt)
    ...
    # menu for mode
    self.Mb = tkinter.Menubutton(
        map_frame, text="Gridding mode", relief=tkinter.RAISED)
    self.Mb.grid(column=0, row=2, pady=(2, 4))
    self.Mb.menu = tkinter.Menu(
        self.Mb, tearoff=0, background='orchid2', fg="black")

    self.Mb.menu.add_command(
        label="NN", command=lambda v=1: _SetMode(master, v))
    self.Mb.menu.add_command(
        label="IDW", command=lambda v=2: _SetMode(master, v))
    self.Mb.menu.add_command(
        label="Gaussian", command=lambda v=3: _SetMode(master, v))
    self.Mb["menu"] = self.Mb.menu
    ...
    self.epsg2_entry = tkinter.Entry(
        map_frame, width=30, textvariable=self.epsg2)
    # , columnspan = 2,sticky = 'EW')
    self.epsg2_entry.grid(column=1, row=2, pady=(2, 4))
    self.epsg2_entry.bind(
        "<Return>", lambda epsg=self.epsg2.get(): _OnPressEnter2(self))
    ...
    resscale = tkinter.Scale(map_frame, variable=self.resvar, from_=0, to=2,
                             resolution=0.01, tickinterval=0.2, label='Resolution [m] 0 = auto')
    resscale.set(0)
    ...
    nnscale = tkinter.Scale(map_frame, variable=self.nnvar, from_=1,
                            to=512, resolution=1, tickinterval=50, label='Nearest neighbours')
    nnscale.set(64)
    ...
    nstdscale = tkinter.Scale(map_frame, variable=self.nstdvar, from_=1,
                              to=10, resolution=1, tickinterval=2, label='Numb. stan. dev.')
    nstdscale.set(5)
...
#	dowrite_entry.configure(background='IndianRed1', fg="black")
    ...
    def _proc4(self):
        ...
        if self.resvar.get() == 0:
            self.resvar = 99
        # do stuff here
        PyHum.map(str(self.DATfilename.get()), os.path.dirname(self.SONfiles[0]), self.epsg2.get(), self.resvar.get(
        ), self.mode, self.nnvar.get(), self.nstdvar.get())  # self.infvar.get(), self.dowritevar.get(),
        self.update()
        messagebox.showinfo("Done!", "Map module finished")
    ...
    lbl6 = tkinter.Label(texture_frame, wraplength='4i',
                         justify=tkinter.LEFT, anchor=tkinter.N, text=''.join(texture_msg))
    ...
    MSG5_btn = tkinter.Button(
        texture_frame, text="Instructions", command=hello5_alt)
    ...
    Winscale2 = tkinter.Scale(texture_frame, variable=self.Winvar2, from_=5,
                              to=500, resolution=1, tickinterval=50, label='Window Size [pixels]')
    Winscale2.set(100)
    ...
    shiftscale = tkinter.Scale(texture_frame, variable=self.shiftvar, from_=2,
                               to=50, resolution=1, tickinterval=10, label='Shift Size [pixels]')
    shiftscale.set(10)
    ...
    densscale = tkinter.Scale(texture_frame, variable=self.densvar,
                              from_=1, to=100, resolution=1, tickinterval=10, label='Density')
    densscale.set(10)
    ...
    maxscalescale = tkinter.Scale(texture_frame, variable=self.maxscalevar,
                                  from_=2, to=50, resolution=1, tickinterval=10, label='Max scale')
    ...
    notesscale = tkinter.Scale(texture_frame, variable=self.notesvar,
                               from_=2, to=20, resolution=1, tickinterval=5, label='Notes')
    notesscale.set(4)
    ...
    ncscale = tkinter.Scale(texture_frame, variable=self.ncvar, from_=2,
                            to=10, resolution=1, tickinterval=2, label='Number classes')
    ncscale.set(4)
    ...
    def _proc5(self):
        ...
        # do stuff here
        PyHum.texture(str(self.DATfilename.get()), os.path.dirname(self.SONfiles[0]), self.Winvar2.get(), self.shiftvar.get(
        ), self.doplot, self.densvar.get(), self.ncvar.get(), self.maxscalevar.get(), self.notesvar.get())

        self.update()
        messagebox.showinfo("Done!", "Texture module finished")
    ...
    MSG6_btn = tkinter.Button(
        map_texture_frame, text="Instructions", command=hello6_alt)
    ...
    #=======================
    # menu for mode
    self.Mbt = tkinter.Menubutton(
        map_texture_frame, text="Gridding mode", relief=tkinter.RAISED)
    self.Mbt.grid(column=0, row=2, pady=(2, 4))
    self.Mbt.menu = tkinter.Menu(
        self.Mbt, tearoff=0, background='coral1', fg="black")

    self.Mbt.menu.add_command(
        label="NN", command=lambda v=1: _SetMode(master, v))
    self.Mbt.menu.add_command(
        label="IDW", command=lambda v=2: _SetMode(master, v))
    self.Mbt.menu.add_command(
    ...
    self.epsg3_entry = tkinter.Entry(
        map_texture_frame, width=30, textvariable=self.epsg3)
    # , columnspan = 2,sticky = 'EW')
    self.epsg3_entry.grid(column=1, row=2, pady=(2, 4))
    self.epsg3_entry.bind(
        "<Return>", lambda epsg=self.epsg3.get(): _OnPressEnter3(self))
    ...
    resscale2 = tkinter.Scale(map_texture_frame, variable=self.resvar2, from_=0,
                              to=10, resolution=0.1, tickinterval=0.5, label='Resolution [m] 0 = auto')
    resscale2.set(0.25)
    ...
    # nn
    self.nnvar = tkinter.DoubleVar()
    nnscale = tkinter.Scale(map_texture_frame, variable=self.nnvar, from_=1,
                            to=512, resolution=1, tickinterval=50, label='Nearest neighbours')
    nnscale.set(64)
    nnscale.grid(row=3, column=1,  pady=(2, 4))
    ...
    # numstdevs
    self.nstdvar = tkinter.DoubleVar()
    nstdscale = tkinter.Scale(map_texture_frame, variable=self.nstdvar,
                              from_=1, to=10, resolution=1, tickinterval=2, label='Numb. stan. dev.')
    nstdscale.set(5)
    nstdscale.grid(row=4, column=1,  pady=(2, 4))
    ...
    def _proc6(self):
        ...
        # do stuff here
        PyHum.map_texture(str(self.DATfilename.get()), os.path.dirname(self.SONfiles[0]), self.epsg3.get(
        ), self.resvar2.get(), self.mode, self.nnvar.get(), self.nstdvar.get())  # self.infvar.get(),

        self.update()
        messagebox.showinfo("Done!", "Map texture module finished")
    ...
    MSG7_btn = tkinter.Button(
        e1e2_frame, text="Instructions", command=hello7_alt)
    ...
    Nscale = tkinter.Scale(e1e2_frame, variable=self.Nvar, from_=2,
                           to=10, resolution=1, tickinterval=2, label='Num. classes')
    Nscale.set(3)
    ...
    # pH
    self.Pvar = tkinter.DoubleVar()
    Pscale = tkinter.Scale(e1e2_frame, variable=self.Pvar,
                           from_=4, to=8, resolution=.1, tickinterval=1, label='pH')
    Pscale.set(7)
    Pscale.grid(row=2, column=1,  pady=(2, 4))
    ...
    # temp
    self.Tvar = tkinter.DoubleVar()
    Tscale = tkinter.Scale(e1e2_frame, variable=self.Tvar, from_=3,
                           to=25, resolution=1, tickinterval=5, label='Temp [degC]')
    Tscale.set(10)
    Tscale.grid(row=3, column=0,  pady=(2, 4))
    ...
    # salinity
    self.Svar = tkinter.DoubleVar()
    Sscale = tkinter.Scale(e1e2_frame, variable=self.Svar, from_=0,
                           to=35, resolution=1, tickinterval=5, label='Salinity [ppt]')
    Sscale.set(0)
    Sscale.grid(row=3, column=1,  pady=(2, 4))
    ...
    BFscale = tkinter.Scale(e1e2_frame, variable=self.BFvar, from_=100,
                            to=500, resolution=5, tickinterval=100, label='Beam freq. [kHz]')
    BFscale.set(200)
    ...
    BAscale = tkinter.Scale(e1e2_frame, variable=self.BAvar, from_=5,
                            to=35, resolution=1, tickinterval=5, label='Beam angle [deg]')
    BAscale.set(20)
    ...
    self.epsg4_entry = tkinter.Entry(
        e1e2_frame, width=30, textvariable=self.epsg4)
    # , columnspan = 2,sticky = 'EW')
    self.epsg4_entry.grid(column=0, row=5, pady=(2, 4))
    self.epsg4_entry.bind(
        "<Return>", lambda epsg=self.epsg4.get(): _OnPressEnter4(self))
    ...
    def _proc7(self):
        ...
        # do stuff here
        PyHum.e1e2(self.DATfilename.get(), os.path.dirname(self.SONfiles[0]), self.epsg4.get(), self.Pvar.get(
        ), self.Tvar.get(), self.Svar.get(), self.BAvar.get(), self.BFvar.get(), self.integ, self.Nvar.get(), self.doplot)

        self.update()
        messagebox.showinfo("Done!", "Bed class finished")
