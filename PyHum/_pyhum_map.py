...
import PyHum.pyhumio as pyhumio
...
try:
    ...
    # print "install pykdtree for faster kd-tree operations:
...
# dogrid = 1, influence = 1, dowrite = 0,
def map(humfile, sonpath, cs2cs_args="epsg:26949", res=99, mode=3, nn=64, numstdevs=5, use_uncorrected=0):
    '''
    ...
    if use_uncorrected == 1:
        ...
        if shape_port != '':
            port_fp = pyhumio.get_mmap_data(
                sonpath, base, '_data_port_l.dat', 'float32', tuple(shape_port))
        if shape_port != '':
            star_fp = pyhumio.get_mmap_data(
                sonpath, base, '_data_star_l.dat', 'float32', tuple(shape_star))

    else:
        if shape_port != '':
            if os.path.isfile(os.path.normpath(os.path.join(sonpath, base + '_data_port_lar.dat'))):
                port_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_port_lar.dat', 'float32', tuple(shape_port))
            else:
                port_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_port_la.dat', 'float32', tuple(shape_port))

        if shape_star != '':
            if os.path.isfile(os.path.normpath(os.path.join(sonpath, base + '_data_star_lar.dat'))):
                star_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_star_lar.dat', 'float32', tuple(shape_star))
            else:
                star_fp = pyhumio.get_mmap_data(
                    sonpath, base, '_data_star_la.dat', 'float32', tuple(shape_star))
    ...
    # read in range data
    R_fp = pyhumio.get_mmap_data(
        sonpath, base, '_data_range.dat', 'float32', tuple(shape_star))
...
#    data_R = R_fp; del R_fp
    ...
    if len(shape_star) > 2:
        for p in range(len(star_fp)):
            try:
                res = make_map(esi[shape_port[-1] * p:shape_port[-1] * (p + 1)], nsi[shape_port[-1] * p:shape_port[-1] * (p + 1)], theta[shape_port[-1] * p:shape_port[-1] * (p + 1)], dist_tvg[shape_port[-1] * p:shape_port[-1] * (
                    p + 1)], port_fp[p], star_fp[p], R_fp[p], meta['pix_m'], res, cs2cs_args, sonpath, p, mode, nn, numstdevs, meta['c'], np.arcsin(meta['c'] / (1000 * meta['t'] * meta['f'])), use_uncorrected)  # dogrid, influence, dowrite,
                print("grid resolution is %s" % (str(res)))
            except:
                print("error on chunk " + str(p))
    else:
        res = make_map(esi, nsi, theta, dist_tvg, port_fp, star_fp, R_fp, meta['pix_m'], res, cs2cs_args, sonpath, 0, mode, nn, numstdevs, meta['c'], np.arcsin(
            meta['c'] / (1000 * meta['t'] * meta['f'])), use_uncorrected)  # dogrid, influence,dowrite,
...
def make_map(e, n, t, d, dat_port, dat_star, data_R, pix_m, res, cs2cs_args, sonpath, p, mode, nn, numstdevs, c, dx, use_uncorrected):  # dogrid, influence,dowrite,
    ...
    yvec = np.squeeze(np.linspace(np.squeeze(pix_m),
                                  extent * np.squeeze(pix_m), extent))
    ...
    np.savetxt(outfile, np.hstack((humutils.ascol(X.flatten()), humutils.ascol(Y.flatten()), humutils.ascol(merge.flatten()), humutils.ascol(
        D.flatten()), humutils.ascol(R.flatten()), humutils.ascol(h.flatten()), humutils.ascol(t.flatten()))), fmt="%8.6f %8.6f %8.6f %8.6f %8.6f %8.6f %8.6f")
    ...
    if 2 > 1:
        ...
        while complete == 0:
            try:
                grid_x, grid_y, res = getmesh(
                    np.min(X), np.max(X), np.min(Y), np.max(Y), resg)
                ...
                if pykdtree == 1:
                    dist, _ = tree.query(
                        np.c_[grid_x.ravel(), grid_y.ravel()], k=1)
                else:
                    try:
                        dist, _ = tree.query(
                            np.c_[grid_x.ravel(), grid_y.ravel()], k=1, n_jobs=cpu_count())
                    except:
                        # print ".... update your scipy installation to use
                        # faster kd-tree queries"
                        dist, _ = tree.query(
                            np.c_[grid_x.ravel(), grid_y.ravel()], k=1)
        ...
        if mode == 1:
            ...
            while complete == 0:
                try:
                    try:
                        dat = pyresample.kd_tree.resample_nearest(orig_def, merge.flatten(
                        ), targ_def, radius_of_influence=res * 20, fill_value=None, nprocs=cpu_count())
                    except:
                        dat = pyresample.kd_tree.resample_nearest(orig_def, merge.flatten(
                        ), targ_def, radius_of_influence=res * 20, fill_value=None, nprocs=1)

                    try:
                        r_dat = pyresample.kd_tree.resample_nearest(orig_def, res_grid.flatten(
                        ), targ_def, radius_of_influence=res * 20, fill_value=None, nprocs=cpu_count())
                    except:
                        r_dat = pyresample.kd_tree.resample_nearest(orig_def, res_grid.flatten(
                        ), targ_def, radius_of_influence=res * 20, fill_value=None, nprocs=1)
                    ...
                    dat, stdev, counts, resg, complete, shape = getgrid_lm(
                        humlon, humlat, merge, res * 20, min(X), max(X), min(Y), max(Y), resg * 2, mode, trans, nn, wf, sigmas, eps)
                    r_dat, stdev, counts, resg, complete, shape = getgrid_lm(
                        humlon, humlat, res_grid, res * 20, min(X), max(X), min(Y), max(Y), resg * 2, mode, trans, nn, wf, sigmas, eps)
                    del humlon, humlat

        elif mode == 2:

            # custom inverse distance
            def wf(r): return 1 / r**2

            complete = 0
            while complete == 0:
                try:
                    try:
                        dat, stdev, counts = pyresample.kd_tree.resample_custom(orig_def, merge.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, weight_funcs=wf, fill_value=None, with_uncert=True, nprocs=cpu_count())
                    except:
                        dat, stdev, counts = pyresample.kd_tree.resample_custom(orig_def, merge.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, weight_funcs=wf, fill_value=None, with_uncert=True, nprocs=1)

                    try:
                        r_dat = pyresample.kd_tree.resample_custom(orig_def, res_grid.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, weight_funcs=wf, fill_value=None, with_uncert=False, nprocs=cpu_count())
                    except:
                        r_dat = pyresample.kd_tree.resample_custom(orig_def, res_grid.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, weight_funcs=wf, fill_value=None, with_uncert=False, nprocs=1)
                    ...
                    humlon, humlat = trans(X, Y, inverse=True)
                    dat, stdev, counts, resg, complete, shape = getgrid_lm(
                        humlon, humlat, merge, res * 2, min(X), max(X), min(Y), max(Y), resg * 2, mode, trans, nn, wf, sigmas, eps)
                    r_dat, stdev, counts, resg, complete, shape = getgrid_lm(
                        humlon, humlat, res_grid, res * 2, min(X), max(X), min(Y), max(Y), resg * 2, mode, trans, nn, wf, sigmas, eps)
                    del humlat, humlon
            ...
            while complete == 0:
                try:
                    try:
                        dat, stdev, counts = pyresample.kd_tree.resample_gauss(orig_def, merge.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=True, nprocs=cpu_count(), epsilon=eps)
                    except:
                        dat, stdev, counts = pyresample.kd_tree.resample_gauss(orig_def, merge.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=True, nprocs=1, epsilon=eps)

                    try:
                        r_dat = pyresample.kd_tree.resample_gauss(orig_def, res_grid.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=False, nprocs=cpu_count(), epsilon=eps)
                    except:
                        r_dat = pyresample.kd_tree.resample_gauss(orig_def, res_grid.flatten(
                        ), targ_def, radius_of_influence=res * 20, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=False, nprocs=1, epsilon=eps)
        ...
        # np.sqrt(np.cos(np.deg2rad(r_dat))) #dat*np.sqrt(r_dat) + dat
        dat = dat + r_dat
    ...
    try:
        ...
        fig.savefig(os.path.normpath(os.path.join(sonpath, 'map' +
                                                  str(p) + '.png')), transparent=True, format='png')
        del fig, ax
        ...
        fig.savefig(os.path.normpath(os.path.join(
            sonpath, 'legend' + str(p) + '.png')), transparent=False, format='png')
        del fig, ax, cs, cb

        # =========================================================
        humutils.make_kml(llcrnrlon=glon.min(), llcrnrlat=glat.min(),
                          urcrnrlon=glon.max(), urcrnrlat=glat.max(),
                          figs=[os.path.normpath(os.path.join(
                              sonpath, 'map' + str(p) + '.png'))],
                          colorbar=os.path.normpath(os.path.join(
                              sonpath, 'legend' + str(p) + '.png')),
                          kmzfile=os.path.normpath(os.path.join(
                              sonpath, 'GroundOverlay' + str(p) + '.kmz')),
    ...
    try:
        map.arcgisimage(server='http://server.arcgisonline.com/ArcGIS',
                        service='World_Imagery', xpixels=1000, ypixels=None, dpi=300)
    except:
        map.arcgisimage(server='http://server.arcgisonline.com/ArcGIS',
                        service='ESRI_Imagery_World_2D', xpixels=1000, ypixels=None, dpi=300)
    # finally:
    #   print "error: map could not be created..."
    ...
    if 2 > 1:
        if datm.size > 25000000:
            ...
            map.pcolormesh(gx[::5, ::5], gy[::5, ::5], datm[::5, ::5],
                           cmap='gray', vmin=np.nanmin(datm), vmax=np.nanmax(datm))
        else:
            map.pcolormesh(gx, gy, datm, cmap='gray',
                           vmin=np.nanmin(datm), vmax=np.nanmax(datm))
        del datm, dat
    else:
        # draw point cloud
        x, y = map.projtran(humlon, humlat)
        map.scatter(x.flatten(), y.flatten(), 0.5,
                    merge.flatten(), cmap='gray', linewidth='0')
...
def custom_save2(figdirec, root):
    plt.savefig(os.path.normpath(os.path.join(figdirec, root)),
                bbox_inches='tight', dpi=1000, transparent=False)
    ...
    while complete == 0:
        try:
            grid_x, grid_y = np.meshgrid(
                np.arange(minX, maxX, res), np.arange(minY, maxY, res))
            if 'grid_x' in locals():
                complete = 1
        except:
    ...
    while complete == 0:
        try:
            ...
            if mode == 1:
                try:
                    dat = pyresample.kd_tree.resample_nearest(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 10, fill_value=None, nprocs=cpu_count())
                except:
                    dat = pyresample.kd_tree.resample_nearest(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 10, fill_value=None, nprocs=1)
                ...
                try:
                    dat, stdev, counts = pyresample.kd_tree.resample_custom(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 10, neighbours=nn, weight_funcs=wf, fill_value=None, with_uncert=True, nprocs=cpu_count())
                except:
                    dat, stdev, counts = pyresample.kd_tree.resample_custom(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 10, neighbours=nn, weight_funcs=wf, fill_value=None, with_uncert=True, nprocs=1)
            else:
                try:
                    dat, stdev, counts = pyresample.kd_tree.resample_gauss(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 10, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=np.nan, nprocs=cpu_count(), epsilon=eps)
                except:
                    dat, stdev, counts = pyresample.kd_tree.resample_gauss(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 10, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=np.nan, nprocs=1, epsilon=eps)
...
def getXY(e, n, yvec, d, t, extent):
    ...
    if os.name == 'posix':
        o = Parallel(n_jobs=cpu_count(), verbose=0)(delayed(xyfunc)(
            e[k], n[k], yvec, d[k], t[k], extent) for k in range(len(n)))

        # eating, northing, distance to sonar, depth, heading
        X, Y, D, H, T = list(zip(*o))
        ...
        for k in range(len(n)):
            out1, out2, out3, out4, out5 = xyfunc(
                e[k], n[k], yvec, d[k], t[k], extent)
            X.append(out1)
            Y.append(out2)
            D.append(out3)
            H.append(out4)
...
if __name__ == '__main__':
    pass
    #list(map(humfile, sonpath, cs2cs_args, res, mode, nn, numstdevs,
...
# return xx, yy, np.sqrt((xx-e)**2 + (yy-n)**2), np.ones(len(xx))*d,
# np.ones(len(xx))*t
    ...
    # else:
    # dat[dist> np.sqrt(1/res) ] = np.nan #np.floor(np.sqrt(1/res))-1 ] =
    # np.nan
