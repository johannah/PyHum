...
def map_texture(humfile, sonpath, cs2cs_args="epsg:26949", res=0.5, mode=3, nn=64, numstdevs=5):  # influence = 10,
    ...
    if len(shape_star) > 2:
        ...
        class_fp = pyhumio.get_mmap_data(
            sonpath, base, '_data_class.dat', 'float32', tuple(shape))
    ...
    if len(shape_star) > 2:
        for p in range(len(class_fp)):
            ...
            outfile = os.path.normpath(os.path.join(
                sonpath, 'x_y_class' + str(p) + '.asc'))
            with open(outfile, 'w') as f:
                np.savetxt(f, np.hstack((humutils.ascol(X), humutils.ascol(
                    Y), humutils.ascol(merge))), delimiter=' ', fmt="%8.6f %8.6f %8.6f")
            ...
            sigmas = 1  # m
            eps = 2
            dat, res = get_grid(mode, orig_def, targ_def, merge, res * 10, np.min(X), np.max(
                X), np.min(Y), np.max(Y), res, nn, sigmas, eps, shape, numstdevs, trans, humlon, humlat)
            ...
            print_map(cs2cs_args, glon, glat, datm,
                      sonpath, p, vmin=vmin, vmax=vmax)
        ...
        if 2 > 1:
            ...
            if pykdtree == 1:
                ...
                try:
                    ...
                    # print ".... update your scipy installation to use faster
                    # kd-tree queries"
                    dist, _ = tree.query(
                        np.c_[grid_x.ravel(), grid_y.ravel()], k=1)
        ...
        Parallel(n_jobs=2, verbose=0)(delayed(doplots)(k, humlon, humlat, cs2cs_args,
                                                       glon, glat, datm, sonpath, 0, vmin=vmin, vmax=vmax) for k in range(2))
...
def doplots(k, humlon, humlat, cs2cs_args, glon, glat, datm, sonpath, p, vmin, vmax):
    ...
    if k == 0:
        ...
        print_contour_map(cs2cs_args, humlon, humlat, glon,
                          glat, datm, sonpath, p, vmin=vmin, vmax=vmax)
...
def print_contour_map(cs2cs_args, humlon, humlat, glon, glat, datm, sonpath, p, vmin, vmax):  # merge,
    ...
    if 2 > 1:
        if datm.size > 25000000:
            ...
            map.pcolormesh(gx[::5, ::5], gy[::5, ::5],
                           datm[::5, ::5], cmap='pink', vmin=vmin, vmax=vmax)
...
def print_map(cs2cs_args, glon, glat, datm, sonpath, p, vmin, vmax):  # humlon, humlat, merge,
    ...
    try:
        ...
        fig.savefig(os.path.normpath(os.path.join(
            sonpath, 'class_map' + str(p) + '.png')), transparent=True, format='png')
        ...
        fig.savefig(os.path.normpath(os.path.join(
            sonpath, 'class_legend' + str(p) + '.png')), transparent=False, format='png')
        ...
        humutils.make_kml(llcrnrlon=glon.min(), llcrnrlat=glat.min(),
                          ...
                          colorbar=os.path.normpath(os.path.join(
                              sonpath, 'class_legend' + str(p) + '.png')),
                          kmzfile=os.path.normpath(os.path.join(
                              sonpath, 'class_GroundOverlay' + str(p) + '.kmz')),
...
def get_grid(mode, orig_def, targ_def, merge, influence, minX, maxX, minY, maxY, res, nn, sigmas, eps, shape, numstdevs, trans, humlon, humlat):
    ...
    if mode == 1:
        ...
        while complete == 0:
            try:
                ...
                dat, res, complete = getgrid_lm(
                    humlon, humlat, merge, res * 10, minX, maxX, minY, maxY, res * 2, mode, trans, nn, wf, sigmas, eps)
        ...
        while complete == 0:
            try:
                ...
                #del grid_x, grid_y, targ_def, orig_def
                dat, stdev, counts, res, complete = getgrid_lm(
                    humlon, humlat, merge, res * 10, minX, maxX, minY, maxY, res * 2, mode, trans, nn, wf, sigmas, eps)
        ...
        while complete == 0:
            try:
                try:
                    ...
                    ), targ_def, radius_of_influence=res * 20, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=np.nan, nprocs=cpu_count(), epsilon=eps)
                except:
                    dat, stdev, counts = pyresample.kd_tree.resample_gauss(orig_def, merge.flatten(
                    ), targ_def, radius_of_influence=res * 20, neighbours=nn, sigmas=sigmas, fill_value=None, with_uncert=np.nan, nprocs=1, epsilon=eps)
                if 'dat' in locals():
                    complete = 1
            except:
                #del grid_x, grid_y, targ_def, orig_def
    ...
    mask = ~binary_dilation(binary_erosion(
        ~mask, structure=np.ones((15, 15))), structure=np.ones((15, 15)))
...
def getXY(e, n, yvec, d, t, extent):
    ...
    try:
        ...
        o = Parallel(n_jobs=1, verbose=0)(delayed(xyfunc)(
            e[k], n[k], yvec, d[k], t[k], extent) for k in range(len(n)))
...
if __name__ == '__main__':
    ...
    map_texture(humfile, sonpath, cs2cs_args, res,
                mode, nn, numstdevs)  # influence,
