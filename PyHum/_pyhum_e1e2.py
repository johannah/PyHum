...
def e1e2(humfile, sonpath, cs2cs_args="epsg:26949", ph=7.0, temp=10.0, salinity=0.0, beam=20.0, transfreq=200.0, integ=5, numclusters=3, doplot=1):
    ...
    equivbeam = (5.78 / (np.power(1.6, 2))) * \
        (np.power((np.sin((beamwidth * np.pi) / (2 * 180))), 2))
    ...
    if shape_hi != '':
        try:
            ...
            with open(os.path.normpath(os.path.join(sonpath, base + '_data_dwnhi.dat')), 'r') as ff:
                dwnhi_fp = np.memmap(
                    ff, dtype='int16', mode='r', shape=tuple(shape_hi))
            ...
            with open(os.path.normpath(os.path.join(sonpath, base + '_data_dwnhi.dat')), 'r') as ff:
                dwnhi_fp = np.memmap(
                    ff, dtype='int16', mode='r', shape=tuple(shape_lo))

    if 'dwnhi_fp' in locals():

        # *(180/pi) # to see in degs
        ...
        if len(shape_hi) > 2:
            ...
            bedi = np.interp(np.linspace(
                1, shape_hi[0] * shape_hi[2], shape_hi[0] * shape_hi[2]), i, bed)
            ei = np.interp(np.linspace(
                1, shape_hi[0] * shape_hi[2], shape_hi[0] * shape_hi[2]), i, es)
            ni = np.interp(np.linspace(
                1, shape_hi[0] * shape_hi[2], shape_hi[0] * shape_hi[2]), i, ns)
            lati = np.interp(np.linspace(
                1, shape_hi[0] * shape_hi[2], shape_hi[0] * shape_hi[2]), i, lat)
            loni = np.interp(np.linspace(
                1, shape_hi[0] * shape_hi[2], shape_hi[0] * shape_hi[2]), i, lon)
            del i
        else:
        ...
        absorption = water_atten(np.asarray(
            depi), transfreq, c, ph, temp, salinity)
        ...
        if len(shape_hi) > 2:
            for p in range(len(dwnhi_fp)):
                ...
                try:  # parallel processing with all available cores
                    w = Parallel(n_jobs=-1, verbose=0)(delayed(get_rgh_hrd)(
                        dwnhi_fp[p][:, i], Zdepi[i], Zabsorp[i], c, nf, transfreq, equivbeam, maxW, pi, ft) for i in ind)
                ...
                f = open(os.path.normpath(os.path.join(
                    sonpath, base + 'rough_and_hard' + str(p) + '.csv')), 'wt')
                writer = csv.writer(f)
                writer.writerow(('longitude', 'latitude', 'easting', 'northing', 'depth', 'roughness',
                                 'hardness', 'average roughness', 'average hardness', 'k-mean label'))
                for i in range(0, len(rough)):
                    writer.writerow((float(Zlon[i]), float(Zlat[i]), float(Zes[i]), float(Zns[i]), float(Zdepi[i]), float(
                        rough[i]), float(hard[i]), float(roughav[i]), float(hardav[i]), labels[i].astype(int)))
                f.close()
                ...
                if doplot == 1:
                    try:
                        ...
                        fig = plt.figure()
                        s = plt.scatter(Zes[labels == 0], Zns[labels == 0],
                                        marker='o', c='k', s=10, linewidth=0, vmin=0, vmax=8)
                        s = plt.scatter(Zes[labels == 1], Zns[labels == 1],
                                        marker='o', c='r', s=10, linewidth=0, vmin=0, vmax=8)
                        s = plt.scatter(Zes[labels == 2], Zns[labels == 2],
                                        marker='o', c='b', s=10, linewidth=0, vmin=0, vmax=8)
                ...
                if doplot == 1:
                    try:
                        ...
                        cs = map.scatter(x.flatten(), y.flatten(
                        ), 1, rough.flatten(), linewidth=0, vmin=0, vmax=8)
                ...
                if doplot == 1:
                    try:
                        ...
                        cs = map.scatter(x.flatten(), y.flatten(
                        ), 1, hard.flatten(), linewidth=0, vmin=0, vmax=8)
                ...
                if doplot == 1:
                    try:
                        ...
                        # draw point cloud
                        x, y = map.projtran(Zlon, Zlat)
                        map.scatter(x.flatten(), y.flatten(), 1,
                                    rough.flatten(), linewidth='0', vmin=0, vmax=8)
                        ...
                        kml.save(os.path.normpath(os.path.join(
                            sonpath, 'Rough' + str(p) + '.kml')))
                ...
                if doplot == 1:
                    try:
                        ...
                        x, y = map.projtran(Zlon, Zlat)
                        map.scatter(x.flatten(), y.flatten(), 1,
                                    hard.flatten(), linewidth='0', vmin=0, vmax=8)
            ...
            if 2 > 1:  # need to tiday all this up later!!
                ...
                try:  # parallel processing with all available cores
                    w = Parallel(n_jobs=-1, verbose=0)(delayed(get_rgh_hrd)(
                        dwnhi_fp[:, i], Zdepi[i], Zabsorp[i], c, nf, transfreq, equivbeam, maxW, pi, ft) for i in ind)
                except:  # fall back to serial
                    w = Parallel(n_jobs=1, verbose=0)(delayed(get_rgh_hrd)(
                        dwnhi_fp[:, i], Zdepi[i], Zabsorp[i], c, nf, transfreq, equivbeam, maxW, pi, ft) for i in ind)
                ...
                #f = open(sonpath+base+'rough_and_hard'+str(p)+'.csv', 'wt')
                f = open(os.path.normpath(os.path.join(
                    sonpath, base + 'rough_and_hard' + str(0) + '.csv')), 'wt')
                writer = csv.writer(f)
                writer.writerow(('longitude', 'latitude', 'easting', 'northing', 'depth', 'roughness',
                                 'hardness', 'average roughness', 'average hardness', 'k-mean label'))
                for i in range(0, len(rough)):
...
def get_rgh_hrd(beamdat, dep, absorp, c, nf, transfreq, equivbeam, maxW, pi, ft):
    ...
    try:
        for k in range(nf, noiseend):  # 80
            backstrength = ((30.0 / 255.0) * (float((np.squeeze(beamdat))[k]))) + (20 * log10(float(dep))) + (2 * (absorp / 1000) * (
                float(dep))) - (10 * (log10(((maxW * (pow((c / (transfreq / 1000)), 2)) * c * 0.0007 * equivbeam) / (32 * (pow(pi, 2)))))))
...
if __name__ == '__main__':
    ...
    e1e2(humfile, sonpath, cs2cs_args, ph, temp, salinity,
         beam, transfreq, integ, numclusters, doplot)
