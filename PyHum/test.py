...
def dotest():
    ...
    shutil.copy(PyHum.__path__[0] + os.sep + 'test.DAT', os.path.expanduser(
        "~") + os.sep + 'pyhum_test' + os.sep + 'test.DAT')

    # general settings
    humfile = os.path.normpath(os.path.join(
        os.path.expanduser("~"), 'pyhum_test', 'test.DAT'))
    sonpath = os.path.normpath(os.path.join(
        os.path.expanduser("~"), 'pyhum_test'))
    ...
    # 1 = apply a phase preserving filter (WARNING!! takes a very long time
    # for large scans)
    dofilt = 0
    # don't retain water column in radiometric correction (1 = retains water
    # column for radiomatric corrections)
    correct_withwater = 0
    ...
    # if res==99, the program will automatically calc res from the spatial res
    # of the scans
    ...
    # influence = 1 #Radius of influence used in gridding. Cut off distance in
    # meters
    ...
    PyHum.read(humfile, sonpath, cs2cs_args, c, draft, doplot, t,
               bedpick, flip_lr, model, calc_bearing, filt_bearing, chunk)  # cog

    # correct scans and remove water column
    PyHum.correct(humfile, sonpath, maxW, doplot, dofilt,
                  correct_withwater, ph, temp, salinity)

    # remove acoustic shadows (caused by distal acoustic attenuation or sound
    # hitting shallows or shoreline)
